...
export default function Home() {
  ...
  const visibleRows = showPending ? rows : rows.filter(r => r.lastChecked);
  ...
  function exportVisibleToExcel() {
    ...
    a.href = url;
    a.download = `backlink_results_${Date.now()}.xlsx`;
          ...
          <div className="mb-4 flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <label className="inline-flex items-center space-x-2 text-sm">
                <input type="checkbox" checked={showPending} onChange={() => setShowPending(s => !s)} className="form-checkbox text-black" />
            ...
            <div className="flex items-center space-x-2">
              ...
              <button onClick={exportVisibleToExcel} className="px-3 py-2 bg-indigo-600 text-white rounded text-sm hover:bg-indigo-700">Export visible results (.xlsx)</button>
            </div>
          </div>
              ...
              <tbody className="bg-white divide-y divide-gray-100">
                {visibleRows.map(r => (
                  <tr key={r.row} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2 text-sm text-black max-w-xs truncate">{r.url}</td>
